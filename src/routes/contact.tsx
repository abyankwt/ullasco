...
export const Route = createFileRoute("/contact")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Contact Ullasco for equipment rental, fleet, transport, and maintenance enquiries in Kuwait. Call, WhatsApp or send a message." },
      { property: "og:title", content: "Contact Ullasco" },
      { property: "og:description", content: "Get in touch for equipment, fleet, transport and maintenance support." },
      ...
      <section className="section-y">
        <div className="container-x grid lg:grid-cols-[1.4fr_1fr] gap-10">
            ...
            {submitted ? (
              <div className="mt-8 rounded-xl bg-primary/10 border border-primary/30 p-6 flex gap-4">
                ...
                <div>
                  ...
                  <p className="text-sm text-muted-foreground mt-1">Your message has been received. Our team will be in touch shortly.</p>
                </div>
              </div>
            ) : (
              <form onSubmit={onSubmit} className="mt-8 grid sm:grid-cols-2 gap-5">
                ...
                <Field label="Company Name"><input name="company" className={inputCls} /></Field>
                <Field label="Phone Number" required><input required type="tel" name="phone" className={inputCls} /></Field>
                ...
                <div className="sm:col-span-2 flex flex-wrap gap-3">
                  ...
                  <a href="https://wa.me/96500000000" target="_blank" rel="noopener noreferrer" className="btn-whatsapp">
                    <MessageCircle className="size-4" /> WhatsApp Us
                  </a>
                  <a href="tel:+96500000000" className="btn-outline-navy"><Phone className="size-4" /> Call Now</a>
                </div>
              </form>
            )}
          </div>
          ...
          <div className="space-y-4">
            <InfoCard icon={MapPin} title="Location" lines={["Kuwait"]} />
            <InfoCard icon={Phone} title="Phone" lines={["+965 0000 0000"]} href="tel:+96500000000" />
            <InfoCard icon={Mail} title="Email" lines={["info@ullasco.com"]} href="mailto:info@ullasco.com" />
            <InfoCard icon={Globe} title="Website" lines={["www.ullasco.com"]} />
            <InfoCard icon={Briefcase} title="Business" lines={["Equipment, Fleet, Transport & Maintenance Services"]} />
          </div>
        </div>
      </section>
...
function InfoCard({ icon: Icon, title, lines, href }: { icon: typeof MapPin; title: string; lines: string[]; href?: string }) {
  const inner = (
    <div className="card-surface p-5 flex gap-4 items-start">
      ...
      <div>
        ...
        {lines.map((l) => <div key={l} className="text-navy font-display font-semibold mt-0.5">{l}</div>)}
      </div>
    </div>
  );
  return href ? <a href={href} className="block hover:translate-y-[-2px] transition">{inner}</a> : inner;
}
